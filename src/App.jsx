...
const generalMessage = 'Hola, tengo un restaurante en [ciudad] y quiero conseguir más clientes. Me gustaría saber qué plan me recomendáis.'
...
const faqs = [
  ['¿Cómo puede GastroVisual ayudarme a conseguir más clientes?', 'Hacemos que tu restaurante tenga una presencia profesional y constante en la web, las redes sociales y Google. Así aumentan las posibilidades de que más personas te encuentren, confíen y contacten o reserven.'],
...
function Brand({ dark = false }) {
  return <a className={`brand ${dark ? 'brand-dark' : ''}`} href="/" aria-label="GastroVisual, inicio"><span>Gastro</span><b>Visual</b><small>by Dania 360</small></a>
}
...
function Hero() {
  ...
  return <section className="hero hero-section" id="inicio" ref={heroRef}>
    <video ref={videoRef} className="hero-media" src="/hero-video-scrub.mp4" muted playsInline preload="auto" poster="/og-image-v2.jpg" style={{ opacity: videoReady ? 1 : 0, transition: 'opacity .5s ease' }} /><div className="hero-shade" />
    <div className="hero-inner"><div className="hero-copy">
      ...
      <p>Ponemos a trabajar tu web, tus redes sociales y Google para que más personas te encuentren, confíen en tu propuesta y contacten contigo.</p>
      <div className="hero-actions"><a className="button button-primary" href={waLink()} target="_blank" rel="noreferrer"><WhatsAppIcon /> Quiero más clientes <ArrowRight size={18} /></a><a className="button button-ghost" href="#planes">Ver planes desde $99</a></div>
    ...
    </div><div className="hero-panel" aria-label="Resumen del servicio GastroVisual">
...
function Services() {
  ...
  })}</div><div className="manifesto"><p>Una agencia genérica vende publicaciones.</p><h2>GastroVisual construye la presencia digital de tu restaurante para generar nuevas oportunidades.</h2></div></section>
}

function PlanCard({ plan }) {
  const message = `Hola, tengo un restaurante en [ciudad] y quiero conseguir más clientes. Me interesa el plan ${plan.name} de GastroVisual. ¿Podéis orientarme?`
...
function Footer() {
  return <><section className="final-cta"><span><i /> ¿Hablamos de tu restaurante?</span><h2>Tu próxima reserva puede empezar con una búsqueda, una publicación o una reseña.</h2><p>Hagamos que tu restaurante esté preparado para aparecer y convencer.</p><a className="button button-primary" href={waLink()} target="_blank" rel="noreferrer"><WhatsAppIcon /> Quiero más clientes <ArrowRight size={18} /></a></section><footer className="footer"><div className="footer-top"><Brand /><p>Servicio especializado en visibilidad digital para restaurantes, desarrollado por Dania 360.</p><div><a href={waLink()} target="_blank" rel="noreferrer">WhatsApp: 631 105 772</a><a href={`mailto:${EMAIL}`}>{EMAIL}</a></div></div><div className="footer-bottom"><span>© {new Date().getFullYear()} GastroVisual · Dania 360</span><nav aria-label="Información legal"><a href="/aviso-legal/">Aviso legal</a><a href="/privacidad/">Privacidad</a><a href="/cookies/">Cookies</a></nav></div></footer><a className="whatsapp-float" href={waLink()} target="_blank" rel="noreferrer" aria-label="Hablar por WhatsApp"><WhatsAppIcon size={25} /><span>¿Hablamos?</span></a></>
}

const pending = <strong className="pending">DATO PENDIENTE DE COMPLETAR</strong>
const legalContent = {
  'aviso-legal': { title: 'Aviso legal', intro: 'Información general sobre el titular y las condiciones de uso de gastrovisual.es.', sections: [
    ['Titular del sitio', <><p>Nombre o razón social: {pending}</p><p>Nombre comercial: GastroVisual · Servicio especializado de Dania 360</p><p>NIF/CIF: {pending}</p><p>Domicilio: {pending}</p><p>Contacto: <a href={`mailto:${EMAIL}`}>{EMAIL}</a> · 631 105 772</p></>],
  ...
  privacidad: { title: 'Política de privacidad', intro: 'Cómo tratamos los datos cuando contactas con GastroVisual.', sections: [
    ['Responsable', <><p>Responsable: {pending}</p><p>Marca comercial: GastroVisual · Dania 360</p><p>Contacto de privacidad: <a href={`mailto:${EMAIL}`}>{EMAIL}</a></p></>],
  ...
  cookies: { title: 'Política de cookies', intro: 'Información sobre el uso de cookies y tecnologías similares.', sections: [
    ['Situación actual', <p>Actualmente gastrovisual.es no instala cookies publicitarias ni utiliza herramientas de analítica que requieran consentimiento. Por este motivo no se muestra un banner de aceptación.</p>],
...
function LegalPage({ type }) {
  ...
  useEffect(() => { document.title = `${content.title} | GastroVisual`; const canonical = document.querySelector('link[rel="canonical"]'); if (canonical) canonical.href = `https://gastrovisual.es/${type}/` }, [content.title, type])
